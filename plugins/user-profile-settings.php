...
<div class="page-login small-form content">
    <form class="" id="<?php echo $selector; ?>" action="" novalidate="novalidate" enctype="multipart/form-data">
        <div class="page-login-input">
            ...
            <input class="dnt-input" name="name" type="text" placeholder="<?php echo $multiLanguage->translate($data, "name", "translate"); ?>" value="<?php echo $user->get()->name ?>" >
        </div>
        <div class="page-login-input">
            <i class="login-icon ion-person"></i>
            <input class="dnt-input" name="surname" type="text" placeholder="<?php echo $multiLanguage->translate($data, "surname", "translate"); ?>" value="<?php echo $user->get()->surname ?>" >
        </div>
        <div class="page-login-input">
            <i class="login-icon ion-at"></i>
            <input class="dnt-input" type="text" name="email" value="<?php echo $user->get()->email ?>" placeholder="<?php echo $multiLanguage->translate($data, "email_addr", "translate"); ?>">
        </div>
        <div class="page-login-input">
            <i class="login-icon ion-flag"></i>
            <input class="dnt-input" type="text" name="krajina" value="<?php echo $user->get()->krajina ?>" placeholder="<?php echo $multiLanguage->translate($data, "country_and_city", "translate"); ?>">
        </div>
        <div class="page-login-input">
...
<?php $date = new DateTime($user->get()->datetime_publish); ?>
        ...
        <div class="page-login-input">
            ...
            <select name="sex" style="padding-bottom: 4px;">
                <option value="1" <?php if ($user->get()->sex == "1") echo "selected"; ?>><?php echo $multiLanguage->translate($data, "man", "translate"); ?></option>
                <option value="2" <?php if ($user->get()->sex == "2") echo "selected"; ?>><?php echo $multiLanguage->translate($data, "woman", "translate"); ?></option>
            </select>
        </div>
        <div class="page-login-input">
            <i class="login-icon fa fa-balance-scale"></i>
            <input class="dnt-input" name="vaha" value="<?php echo ($user->get()->vaha == 0) ? "" : $user->get()->vaha ?>" placeholder="<?php echo $multiLanguage->translate($data, "weight", "translate"); ?> (kg)" type="number">
        </div>
        <div class="page-login-input">
            <i class="login-icon fa fa-arrow-up"></i>
            <input class="dnt-input" name="vyska" value="<?php echo ($user->get()->vyska == 0) ? "" : $user->get()->vyska ?>" placeholder="<?php echo $multiLanguage->translate($data, "height", "translate"); ?> (cm)" type="number">
        </div>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        jQuery("#<?php echo $selector; ?>").validate({
            ...
            messages: {
                name: "<?php echo $multiLanguage->translate($data, "required_field", "translate"); ?>",
                email: "<?php echo $multiLanguage->translate($data, "required_field", "translate"); ?>",
            },
